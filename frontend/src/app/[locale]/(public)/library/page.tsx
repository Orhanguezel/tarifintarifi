...
import { fetchLibraryListServer } from "@/lib/library/api.server";
import type { ILibrary, TranslatedField } from "@/lib/library/types";

export const revalidate = 300; // 5 dk

// İçerik için strict dil (fallback yok)
...
export default async function LibraryPage({ params }: { params: Params }) {
  ...
  let items: ILibrary[] = [];
  try {
    items = await fetchLibraryListServer({ locale, limit: 12, sort: "-publishedAt" });
  } catch {
    items = [];
  }
  ...
  return (
    <SectionScaffold
      h1={t("title", { default: "Library" })}
      intro={t("intro", { default: "Browse our technical library (PDFs, guides, datasheets)." })}
      breadcrumbs={[
    ...
    >
      {items.length === 0 ? (
        <p style={{ color: "#666" }}>
          {t("empty", { default: "No documents yet. Please check back soon." })}
        </p>
      ) : (
        <div
          style={{
        ...
        >
          {items.map((it) => {
            ...
            const href = `/${locale}/library/${encodeURIComponent(it.slug)}`;
            const thumb =
