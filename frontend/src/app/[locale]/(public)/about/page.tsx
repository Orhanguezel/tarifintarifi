...
import { fetchAboutListServer } from "@/lib/about/api.server";
import type { IAbout, TranslatedField } from "@/lib/about/types";

export const revalidate = 300; // 5 dk

// ── İçerik için SIKI dil okuma (fallback yok) ─────────────────────────────────
function pickTextStrict(tf: TranslatedField | undefined, locale: SupportedLocale): string {
...
export default async function AboutPage({ params }: { params: Params }) {
  ...
  let items: IAbout[] = [];
  try {
    items = await fetchAboutListServer({ locale, limit: 12, sort: "-publishedAt" });
  } catch {
    items = [];
  }
  ...
  return (
    ...
    >
      {items.length === 0 ? (
        <p style={{ color: "#666" }}>
          {t("empty", { default: "No records yet. Please check back soon." })}
        </p>
      ) : (
        <div
          style={{
        ...
        >
          {items.map((it) => {
            const title = pickTextStrict(it.title, locale);
            const summary = pickTextStrict(it.summary, locale);

            // ✅ pretty URL: /{locale}/about/{slug}
            const href = `/${locale}/about/${encodeURIComponent(it.slug)}`;
              ...
              >
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  {thumb ? (
                    <img
                      src={thumb}
                      alt={title || it.slug /* alt boş kalmasın */}
                      width={80}
                      height={60}
