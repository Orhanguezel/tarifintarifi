// app/[locale]/(public)/products/[slug]/page.tsx
...
export async function generateMetadata(
  ...
  const item = await fetchProductBySlugServer(slug, locale);
  if (!item) {
    return basicMeta({
      locale,
      path: `/${locale}/products/${encodeURIComponent(slug)}`,
      title: t("title", { default: "Products" }),
      description: t("desc", { default: "Explore our industrial products." }),
    });
  }
  ...
  return basicMeta({
    ...
    path: `/${locale}/products/${encodeURIComponent(slug)}`,
    title: name,
    description: clip(desc) || t("desc", { default: "Explore our industrial products." }),
  });
}
...
export default async function ProductDetailPage(
  ...
  if (!slug) redirect(`/${locale}/products`);
  if (!slugFromParams && slug) redirect(`/${locale}/products/${encodeURIComponent(slug)}`);

  const t = await getSafeT(locale);

  const item: IEnsotekprod | null = await fetchProductBySlugServer(slug!, locale);
  if (!item) notFound();
  ...
  return (
    <SectionScaffold
      ...
      breadcrumbs={[
        ...
        { name, url: `/${locale}/products/${encodeURIComponent(slug!)}` },
      ]}
    >
      {hero ? (
      ...
      {/* (İleride) zengin açıklama alanı / teknik dökümanlar vs. */}
    </SectionScaffold>
  );
}
