...
type Props = {
  ...
  h1Text?: string; // dışarıdan H1 metni gelebilir
};
...
export default function HomeView({
  ...
  useEffect(() => {
    ...
    if (selectedCat) usp.set("cat", normalizeCat(selectedCat));
    else usp.delete("cat");
  ...
  useEffect(() => {
    ...
    if (page > total) {
      const usp = new URLSearchParams(params.toString());
      if (total > 1) usp.set("page", String(total));
  ...
  const go = (p: number) => {
    ...
    setPage(next);
    const usp = new URLSearchParams(params.toString());
    if (next > 1) usp.set("page", String(next));
    else usp.delete("page");
    router.replace(`${pathname}?${usp.toString()}`, { scroll: false });
  };
  ...
  /* latest — önce SSR fallback, sonra API */
  type LatestView = {
    id: string;
    title: string;
    href: string;
  ...
  const mapLatest = (r: Recipe): LatestView => {
    const title =
      (r.title as any)?.[locale] ||
      r.title?.tr ||
      (r as any).slugCanonical ||
      "Tarif";
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const baseDedup = pageItemsSortedDesc.filter(
    ...
    <Container>
      ...
      {hasFilters && (
        <>
          ...
          {!loading && !err && pageItemsSortedDesc.length === 0 && (
            <Muted>{t("empty")}</Muted>
          )}

          {!!pageItemsSortedDesc.length && (
            <RecipeList items={pageItemsSortedDesc} locale={locale} />
          )}
      ...
      {!hasFilters && (
        <>
          ...
          <List>
            {latest.map((r) => (
              <ListItem key={r.id}>
                <NextLink prefetch={false} href={r.href}>
                  {r.title}
                </NextLink>
                <ListMeta>
                  <span>
                    ⏱ {r.minutes} {tc("unit.minutesShort")}
                  </span>
                  <span>
                    🔥 {r.calories} {tc("unit.kcal")}
                  </span>
                  <span>❤️ {r.likes}</span>
